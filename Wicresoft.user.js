...
function autoPlayCourse()
{
    ...
    if (getAliPlayerFrame())
    {
        ...
        // 监听下一节按钮并自动点击
        autoClickNextButton(iframe);
    } //else { console.log("未找到aliPlayerFrame"); }
    else
    {
        var old_iframe = document.getElementById("iframe_aliplayer");
        var old_iframeDocument = old_iframe.contentDocument;
        var old_isVideoPlaying = false; // 记录视频是否正在播放的状态
        if (old_iframe && old_iframeDocument)
        {
            console.log("找到旧版aliPlayerFrame");
            var old_videoObserver = new MutationObserver(function (mutations)
            {
                mutations.forEach(function (mutation)
                {
                    var old_video = old_iframeDocument.querySelector("video");
                    if (mutation.type === "childList" && old_video)
                    {
                        ...
                        // 添加监听暂停事件的逻辑
                        old_video.addEventListener("pause", function ()
                        {
                            ...
                            old_isVideoPlaying = false; // 更新视频状态为暂停
                        });
                    }
            ...
            old_videoObserver.observe(old_iframeDocument.body, {
                childList: true,
                subtree: true,
            });
        }
        else { console.log("未找到旧版aliPlayerFrame"); };
...
{
    console.log("寻找aliPlayerFrame");
    var iframe = document.getElementById("aliPlayerFrame");
    if (iframe && iframe.contentDocument)
    {
        console.log("找到aliPlayerFrame");
        console.log("aliPlayerFrame加载完毕");
        return iframe;
    }
}
...
// 监听下一节按钮并自动点击
function autoClickNextButton(iframe)
{
    setVideoPlaybackRate(iframe);
